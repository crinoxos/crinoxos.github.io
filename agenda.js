...
(function () {
  ...
  function extractBlocksFromSimpleJson(data) {
    ...
    data.eventos.forEach(function (ev) {
      ...
      // Parseamos manualmente para asegurar que se trate como hora local (evitando desfases UTC)
      var s = parseSqlDateTime(ev.inicio.replace('T', ' '));
      var e = parseSqlDateTime(ev.fin.replace('T', ' '));
